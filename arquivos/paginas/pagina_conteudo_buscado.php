<?php
    ...
    if(isset($_GET['procurar_txt'])){
        $txt_busca = $_GET['procurar_txt'];
        $txtbuscaquery = "SELECT * FROM cadastro_componentes WHERE nome LIKE '" . $txt_busca . "%' OR categoria LIKE '" . $txt_busca . "%' OR genero LIKE '" . $txt_busca . "%' OR sinopse LIKE '" . $txt_busca . "%' OR data_registro LIKE '" . $txt_busca . "%'";
        $txtbuscaqueryres = $conn->query($txtbuscaquery);

        $linhas_retorno = mysqli_num_rows($txtbuscaqueryres);
    }else{
        header("Location: ../logica/php/delogar.php");
    }
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Mundo Leitura | Conteúdo Buscado</title>
</head>
    <body>
            ...
            <section class="divisao_conteudo">
                ...
                <section class="conteudo_principal_pagina">
                    <section class="header_conteudo">
                        <h1>FORAM ENCONTRADOS: <?php echo($linhas_retorno); ?> RESULTADOS PELA BUSCA DE: <?php echo($txt_busca); ?></h1>
                    </section>

                        <section class="cartoes_conteudo">
                            <?php 
                                while($reslivros = $txtbuscaqueryres->fetch_assoc()){
                                    $id = $reslivros['id'];
                                    $nome = $reslivros['nome'];
                        ...
                        <section class="cartao_solo">
                            ...
                            <section class="descricao_conteudo">
                                ...
                                <h4><a href="#">SABER MAIS SOBRE</a></h4>
                            </section>
                        </section>
                        <?php } ?>
                    </section>
                ...
                <section class="conteudo_novidades">
                    ...
                    <section class="conteudo_destaque">
                        <?php 
                            $buscalivro = "SELECT * FROM cadastro_componentes WHERE categoria = 'Filmes' LIMIT 4";
                            $buscalivrores = $conn->query($buscalivro);
                            while($reslivros = $buscalivrores->fetch_assoc()){
                                $id = $reslivros['id'];
                                $nome = $reslivros['nome'];
